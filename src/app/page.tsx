      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-8 sm:p-24 text-center">
        <div className="z-10 w-full max-w-2xl flex flex-col items-center gap-4">
          ...
          <p className="mt-4 text-sm">
            to get started, <Link href="/login" className="underline">log in</Link> and customize your site via the <Link href="/settings" className="underline">settings</Link> page.
          </p>
        </div>
      </main>
    );
      ...
      <div className="z-10 w-full max-w-2xl flex flex-col gap-12">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
          ...
          <div>
            ...
            <p className="text-md text-gray-600 dark:text-gray-400 mt-1">{profile.bio}</p>
          </div>
        </div>
        ...
        {profile.connections && Object.keys(profile.connections).length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-black dark:text-white">connections</h2>
            <hr className="my-2 border-gray-200 dark:border-gray-700" />
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              {Object.entries(profile.connections).map(([key, value]) => (
                ...
                >
                  {key}
                  <FiArrowUpRight className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" size=".75em" />
                </Link>
              ))}
            </div>
          </div>
        ...
        {latestPosts && latestPosts.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-black dark:text-white">latest updates</h2>
            <hr className="my-2 border-gray-200 dark:border-gray-700" />
            <div className="flex flex-col gap-4">
              {latestPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
                  <h3 className="font-semibold text-black dark:text-white group-hover:underline">{post.title}</h3>
